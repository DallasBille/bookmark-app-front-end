...
export default class ArticleContainer extends Component {
  ...
  handleMoveToRead = readArticle => {
    ...
    ArticleAdapter.updateUnreadToRead(readArticle).then(article => {
      this.setState({
  ...
  handleMoveToUnread = unreadArticle => {
    ...
    ArticleAdapter.updateReadToUnread(unreadArticle).then(article => {
      this.setState({
  ...
  submitNewArticle = (e, newArticle) => {
    e.preventDefault();
    ArticleAdapter.postArticle(newArticle).then(article => {
      this.setState({
  ...
  updateNotes = (event, noteState, articleObj) => {
    ...
    const filteredList = this.state.articles.filter(
      article => article.id !== articleObj.id
    );
    ArticleAdapter.updateNotes(noteState, articleObj).then(article => {
      this.setState({
  ...
  sortArticles = () => {
    ...
    const filtered = this.state.articles.filter(article => {
      return (
        article.category === filterState || article.urgency === filterState
      );
    });
            ...
            render={renderProps => {
              return (
                <UnreadList
                  updateNotes={this.updateNotes}
                  deleteArticle={this.deleteArticle}
                  handleMoveToRead={this.handleMoveToRead}
                  unreadArticles={this.unreadArticles()}
                />
              );
            }}
          />
            ...
            render={renderProps => {
              return (
                <ReadList
                  updateNotes={this.updateNotes}
                  deleteArticle={this.deleteArticle}
                  handleMoveToUnread={this.handleMoveToUnread}
